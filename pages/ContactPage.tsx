      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
        <AnimateOnScroll className="lg:pt-8 mb-12 lg:mb-0">
          ...
          <div className="mt-12 space-y-6 text-lg">
            <div>
                ...
                <a href={`mailto:${CONTACT_INFO.email}`} className="text-brand-light hover:text-brand-red transition-colors text-2xl font-medium">{CONTACT_INFO.email}</a>
            </div>
            <div>
        ...
        <AnimateOnScroll delay={100} className="bg-gray-900 border border-gray-800 rounded-2xl p-8 lg:p-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {status && <p className={`text-center mt-4 ${status.includes('Thank') || status.includes('Hvala') ? 'text-green-400' : 'text-red-400'}`}>{status}</p>}
          </form>
        </AnimateOnScroll>
      </div>
    </PageWrapper>
